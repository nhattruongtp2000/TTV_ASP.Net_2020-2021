    ...
    public class OrdersController : Controller
    {
        ...
        public async Task<ActionResult> Excel()
        {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(1, 1).Value = "Mã Đặt";
                worksheet.Cell(1, 2).Value = "Tên tài khoản";
                worksheet.Cell(1, 3).Value = "Tổng Giá";
                worksheet.Cell(1, 4).Value = "Ngày Đặt";
                worksheet.Cell(1, 5).Value = "Mã Voucher";
                ...
                for (int index = 1; index <= products.Count(); index++)
                {
                    ...
                    worksheet.Cell(index + 1, 5).Value = products[index - 1].VoucherCode;
                    worksheet.Cell(index + 1, 6).Value = products[index - 1].Status;
